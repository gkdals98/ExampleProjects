...
+ 왜 mixin이 안먹히는거지;; 없으면 sass 의미 반쪽되는데.... 일단 이거 해결도 중요하다.
...
+ 다만 지금 놓치는 부분이 있을지도 모르니 우선은 와꾸만들기 작업부터 쭉 진행하고 하자.
------
#### Data
+ 데이터 정의를 합시다 역시.
+ 컴포넌트를 늘리는건 역시 좀 그러려나? App이 구조체로 다 들고있는게 나을 것 같기도 하다.
+ 정의가 다 된 다음에 필요한 항목을 한정하고 필요없는 v-bind를 걷어낸다.
+ 데이터가 변할 때 animation에 관한 메모. +100 과 같은 문구를 게임 창 아래에서 표시하는게 나을 것 같다...
+ static data
    * 가진 캐릭터들의 원래 스텟
    * 가진 아이템들의 원래 스텟
+ current data
