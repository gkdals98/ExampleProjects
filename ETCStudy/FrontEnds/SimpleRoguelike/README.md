...
2. 가챠타임이 오고 가진 자원만큼 가챠를 돌려 리스트가 쭉 나온다. 원하는 캐릭터를 선택해 자리에 배치한다. 원래 스테이지당 한 명 합류로 하려했으나 이게 합리적이라 봄.
...
------
#### Core
* 이제 게임의 전체 흐름을 관리하는 GameController를 만들 때가 왔다.
* 구체적으론 현재가 어느 Scene인지, 진행도가 어떠니 어떤 버튼을 눌렀을 땐 어떤 동작을 해야하는지를 정하는 녀석이다.
* 척추같은 녀석인데 당장은 감이 잘 안오더라도 우선 만들고 리펙토링하던가 하자. 이대로 무척추생물 상태로 갈 순 없다.
##### 1. 기본적인 흐름, 한 번 더 정리.
* 목적이 확고해야 코드가 안 꼬인다. GameFrame에서 Scene전환은 알아서 하고, 각 Scene의 Controller에서 다음 Scene으로 넘어갈 조건이 됐는지를 판단하는데 어째서 컨트롤러가 필요한가?
* 각 Scene으로 넘어가기 전, 사전 준비를 할 타이밍을 잡기 위해서, 또 수행여부를 전체 흐름을 놓고 판단해야하는 경우를 위해서이다. 대표적으로 스테이지를 넘어가 맵이 젠되는 부분이 그러하다.
* 또한 언제든 발생할 수 있는 GameOver의 경우, 통합된 전체 Controller에서 관리할 필요가 있다.
------
#### Data
...
+ 의존성 꼬임...... vue cli프로젝트의 기본 의존성과 비교하여 원상복구하되 기본의존성에 없는 것들이 무엇이 있는지, 그 중 필요한게 무엇인지 반드시 체크해야한다.


+ Hold. 현재 Json 읽어오는데에서 멈췄다.
...
+ JSON 파일 읽는 방법에 참조할 링크들. 리소스는 JSON 별도 파일로 관리해서 원하는대로 커스터마이징 가능하게 해보자. - https://aljjabaegi.tistory.com/327   http://blog.naver.com/PostView.nhn?blogId=javaking75&logNo=140174605495